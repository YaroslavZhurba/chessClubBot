...
def send_question(user, amount):
    ...
    return tgbot.send_message(user_handler.get_user_chat_id(user),
...
def start_quiz(user):
    ...
    user_handler.set_user_substate(user, 0)
    quiz_no = random.randint(0, quiz_collection.db["amount"] - 1)
    user_handler.set_user_random_state(user, quiz_no)
    quiz = quiz_collection.db["quizzes"][quiz_no]
    quiz_results_handler.set_zero_score(user_handler.get_user_chat_id(user), quiz_no)
    return send_question(user, quiz_handler.get_amount(quiz))


def continue_quiz(user, command):
    quiz_result, pos = quiz_results_collections.get_by_chat_quiz(user_handler.get_user_chat_id(user),
                                                                 quiz_handler.get_id(
                                                                     quiz_collection.get_user_quiz(user)))
    if check_answer(user, command):
        quiz_results_handler.add_score(pos)

    substate = user_handler.get_user_substate(user) + 1
    user_handler.set_user_substate(user, substate)
...
def end_quiz(user, quiz_result, max_result):
    user_handler.set_user_state(user, configs.States.default)
    return tgbot.send_message(user_handler.get_user_chat_id(user),
